...
function CustomerProfilePage()
{
	...
	const handleDeleteNotif = async (e,d,i) => {
		e.preventDefault()
		try {
	...
	return (
		<div className="col ms-4 me-4 rounded-5 p-4" style={{backgroundColor:"rgba(255,255,255,.35)",height:"100%",overflow:"auto"}}>
			...
			<div className="d-flex flex-column gap-3 pt-3">
					...
					<h5 className={"text-center text-white "+((!data.length)?"d-block":"d-none")} style={{textShadow:"2px 2px 2px black"}}>Empty notifications</h5>
					{
						data.map((d,i)=>(
							<div className="col d-flex">
								...
								<button type="button" className="btn btn-danger rounded-0 rounded-end-2 p-4" data-bs-toggle="modal" data-bs-target={"#notifDC"+i}>
				...
				{
					data.map((d,i)=>(
						<div className="modal fade" tabIndex="-1" id={"notifDC"+i}>
							<div className="modal-dialog modal-dialog-centered">
								<form className="modal-content" onSubmit={(e)=>{handleDeleteNotif(e,d,i)}}>
									...
									<div className="modal-footer">
										<button type="button" className="btn btn-dark" id={"notifNoDelete"+i} data-bs-dismiss="modal">No</button>
