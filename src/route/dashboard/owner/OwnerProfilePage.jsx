...
function OwnerProfilePage()
{
	...
	const handleChangeProfile = (e) => {
		...
		reader.onload = async () => {
			try {
				...
				console.error(e)
				setSaveSuccess("")
				setSaveError((window.navigator.onLine)?"Unable to load profile picture. Reason: Server error.":"Unable to save profile picture. Reason: no internet connection.")
			}
		}
	}
	const handleSubmit = async (e) => {
		...
		try {
			...
			setLoading(false)
			setSaveSuccess("")
			setSaveError((window.navigator.onLine)?"Unable to save. Reason: No internet connection":"Unable to save. Reason: Server error.")
		}
	}
	...
	useEffect(()=>{
		async function fetchData() {
			try {
				...
				de("myProfile").src = (response.data.profile !== null)?response.data.profile:"https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png" 
			} catch (e) {
				console.error(e)
				setSaveError((window.navigator.onLine)?"Unable to load user information. Reason: Server error.":"Unable to load user information. Reason: no internet connection.")
			}
		}
		fetchData()
	},[])
			...
			<form className="container-fluid p-2 d-flex flex-column" onSubmit={handleSubmit}>
				<div className="row row-cols-1 row-cols-md-2">
					...
					<div className="col col-md-8 text-white" style={{textShadow:"2px 2px 2px black"}}>
						...
						<div className="input-group">
							<input id="myEmailInput" className="form-control" placeholder="example@gmail.com" required />
